...
def add():
    CSV.initialize_csv()
    date = get_date(
        "Enter the date of the transaction (dd-mm-yyyy) or click enter for today's date: ", allow_default=True)
    amount = get_amount()
...
def plot_transactions(df):
    ...
    # first param is x axis second is Y axis
    plt.plot(income_df.index, income_df["amount"], label="Income", color="g")
    plt.plot(expense_df.index,
             expense_df["amount"], label="Expense", color="r")
    plt.xlabel("Date")
    plt.ylabel("Amount")
    plt.title('Income and Expenses for Provided Dates')
    # enables colored labels for income and expense
    plt.legend()
    # enables grid lines
    plt.grid(True)
    plt.show()
...
# only execute if main is called for execution
